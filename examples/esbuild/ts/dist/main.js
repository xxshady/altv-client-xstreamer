...
var streamer_default = new Worker("./altv-client-xstreamer__streamer.worker.js");
...
var _Streamer = class {
  ...
  eventHandlers = {
    ["streamResult"]: (streamOut, streamIn, mainStream) => {
  ...
  removeEntity(entity) {
    this.emitWorker("destroyEntity", entity.id);
  }
  initEvents() {
    for (const eventName in this.eventHandlers)
      streamer_default.on(eventName, this.eventHandlers[eventName]);
  }
  emitWorker(eventName, ...args) {
    streamer_default.emit(eventName, ...args);
  }
  runMainStream() {
    const {
      pos: { x, y }
    ...
    this.oldPos = { x, y };
    this.emitWorker("stream", this.oldPos);
  }
  async startEntityCreateQueue() {
    ...
    while (entities.length > 0) {
      ...
      this.emitWorker("createEntities", entitiesToSend);
...
var Marker = class extends Entity {
  constructor(pos, type = 1, _scale = 0.5) {
    super(pos, 5);
    this.type = type;
    this._scale = _scale;
  }
...
__decorateClass([
  validEntity()
], Marker.prototype, "scale", 1);
Marker = __decorateClass([
  defineEntityPool({
    maxStreamedIn: 2,
    onStreamIn: (marker2) => marker2.streamIn(),
    onStreamOut: (marker2) => marker2.streamOut()
  })
], Marker);
